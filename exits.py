...
async def exit1(robot: Robot):
    ...
    launch1TurnSettings = LaunchSettings(kp = 8, ki = 3, kd = 0.8, safetyThreshold = 1200, minSpeed=30)
    ...
    await gyroStraightRotations(robot, launch1StraightSettings, 2, 40, 80)
    await gyroPivot(robot, launch1TurnSettings, front, 20)
    await gyroStraightRotations(robot, launch1StraightSettings, 0.1, 20, 50)
    await robot.leftAttachment.run_time(-1000, 1550)
    await gyroPivot(robot, launch1TurnSettings, back, -5)


    await gyroStraightRotations(robot, launch1StraightSettings, 0.75, -5, 50)
    ...
    await wait(600)
    await gyroStraightTime(robot, launch1StraightSettings, 1.4, 86, 60)
    robot.rightAttachment.run_time(-1000 , 1000, wait=False)
    await gyroStraightTime(robot, launch1StraightSettings, 1.6, 90, -30)
...
async def sampleExit3(robot: Robot):
    ...
    await gyroStraightTime(robot, launch3StraightSetings , 0.8 , 1 , -100)
    await robot.leftAttachment.run_time(-700, 500)
    await gyroStraightTime(robot , launch3StraightSetings , 0.6 , 0 , 100)
    return exitTimer.time()
...
async def sampleExit6(robot: Robot):
    ...
    launch6TurnSettings2 = LaunchSettings(kp = 6, ki = 3, kd = 0.8, safetyThreshold = 1200, minSpeed=40)
    ...
    await gyroPivot(robot, launch6TurnSettings2, front, 0)

    await gyroStraightTime(robot, launch6StraightSettings, 1.1, 0, 80)
    robot.rightAttachment.stop()
...
async def sampleExit9(robot: Robot):    
    ...
    await resetGyro(robot)
    await waitForStart(robot)
    launch9StraightSetings = LaunchSettings(kp = 4, ki = 0, kd = 0.09)
    ...
    launch9TurnSetings2 =  LaunchSettings(kp = 6, ki = 3, kd = 0.8, safetyThreshold = 1600, minSpeed=40)
    ...
    await gyroStraightRotations(robot, launch9StraightSetings , 1.2 , -10, 100)
    await gyroStraightRotations(robot, launch9StraightSetings , 1 , 0, 100)
    await gyroSpin(robot, launch9TurnSettings3, 30, 2)


    robot.leftDrive.run_time(1000 , 1700 , wait=False)
    await robot.rightDrive.run_time(1000 , 1700)
    robot.rightAttachment.dc(0)
    robot.rightAttachment.stop()
    await robot.rightAttachment.run_time(-1000 , 500)
    await gyroStraightTime(robot, launch9StraightSetings , 0.4 , -1 , -80)
    await gyroStraightTime(robot, launch9StraightSetings , 0.6 , 5 , -70)
    await gyroStraightTime(robot, launch9StraightSetings , 0.7 , 0 , -70)
    ...
    await gyroStraightRotations(robot, launch9StraightSetings , 1.45 , -55 , 60)
    ...
    await robot.rightAttachment.run_time(-1000 , 1700) 
    await wait(300)
